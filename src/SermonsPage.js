...
// // import PdfJsModal from './PdfJsModal';
...
// import Sidebar from './Sidebar';
// import './SermonsPage.css';
// // ⬇️ PdfViewerModal బదులుగా PdfJsModal వాడుతున్నాం
// import PdfJsModal from './PdfJsModal';
...
//         const { data } = await axios.get(`${API_BASE}/api/sermons`);
//         setSermons(data?.sermons || []);
//         // [PATCH] 'All' డూప్లికేట్లు రాకుండా unique lists
//         const uniqAuthors = Array.from(new Set(['All', ...(data?.authors || [])].filter(Boolean)));
//         const uniqCategories = Array.from(new Set(['All', ...(data?.categories || [])].filter(Boolean)));
...
//       {/* Modal mount (PDF.js based) */}
//       <PdfJsModal open={isModalOpen} url={modalUrl} title={modalTitle} onClose={closePdf} />
//     </div>
//   );
// }
...
function SermonsPage() {
  ...
  const filteredSermons = useMemo(
    () =>
      sermons.filter((sermon) =>
        ...
        (selectedAuthor === 'All' || sermon.author === selectedAuthor) &&
        (selectedAlphabet === '' ||
          (sermon.title || '').toLowerCase().startsWith(selectedAlphabet.toLowerCase())) &&
        (sermon.title || '').toLowerCase().includes(searchTerm.toLowerCase())
      ),
    [sermons, selectedCategory, selectedAuthor, selectedAlphabet, searchTerm]
  );
  ...
  if (loading) return <div className="sermons-container">లోడ్ అవుతోంది...</div>;
  if (error) return <div className="sermons-container error-message">{error}</div>;
  ...
  return (
    <div className="main-layout">
      ...
      <div className="sermons-content">
        <h2>ప్రసంగాలు</h2>
        <p className="page-description">ఇక్కడ మీరు పాస్టర్ల ప్రసంగాలను పీడీఎఫ్ ఫైల్స్‌లో చదవవచ్చు.</p>
              ...
              <div key={sermon._id} className="sermon-card">
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  ...
                  <a
                    ...
                    style={{
                      background: '#16a34a',
                      pointerEvents: sermon.pdf_url ? 'auto' : 'none',
                      opacity: sermon.pdf_url ? 1 : 0.5,
                    }}
                  >
                    డౌన్‌లోడ్
                  </a>
